...
const AddProductModal = ({ onClose }) => {
  ...
  const handleQuantityUnitChange = e => {
    const v = e.target.value;
    const partialUnitPattern = /^\d*(?:k(?:g)?|m(?:l)?|g|l)?$/i;
  
    if (!partialUnitPattern.test(v)) {
      setQuantityUnitError("Use format like '500g', '1kg', '250ml' or '1L'.");
    } else {
      setQuantityUnit(v);
  ...
  const validateQuantityUnit = () => {
    ...
    const fullUnitPattern = /^\d+(?:g|kg|l|ml)$/i;
  
    if (!trimmed) {
      setQuantityUnitError("Quantity unit is required.");
      return false;
    }
    if (!fullUnitPattern.test(trimmed)) {
      setQuantityUnitError("Quantity unit must be like '500g', '1kg', '250ml' or '1L'.");
      return false;
    }
    setQuantityUnitError("");
    return true;
        ...
        <div className="select-category-section">
          ...
          <div className="category-scroll-wrapper">
            <div className="category-scroll" ref={scrollRef}>
              ...
              <button
                key={cat._id}
                className={`category-tab ${activeCategory?._id === cat._id ? "active" : ""}`}
                onClick={() => {
            ...
            <span className="scroll-arrow" onClick={scrollRight}>
              &gt;
            </span>
          </div>
